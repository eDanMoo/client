...
socket.on("join_answer", async (res) => {
  ...
  if (res.joined) {
    console.log(
      `This peer has been joined to the room: ${res.roomId}. SocketId: ${socket.id}`
    );
...
socket.on("webrtc_offer", async (msg) => {
  // Acting as remote responser
...
socket.on("webrtc_answer", (msg) => {
  // Acting as offerer initiator
...
async function newPeer(socketId) {
  ...
  localStream.getTracks().forEach((track) => {
    const transceiver = peer.addTransceiver(track, {
      direction: "sendonly",
      streams: [localStream],
    }); // Same as previous line without usurpation
  });
  ...
  peer.addEventListener("connectionstatechange", async (e) => {
    if (peer.connectionState === "connected") {
      ...
      console.log(
        `client peer.connectionState='failed'. It should never reach here`
      );
    }
  });
  peer.addEventListener(
    "track",
    ({ transceiver, receiver, streams: [stream], track }) => {
